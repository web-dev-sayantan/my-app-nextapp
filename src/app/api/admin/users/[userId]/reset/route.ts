...
export async function POST(req: Request, props: { params: Promise<{ userId: string }> }) {
  ...
  try {
    ...
    const adminUserId = (session as any).user?.id;
    if (adminUserId === params.userId) {
...
if (!user) {
  return NextResponse.json({ error: 'User not found' }, { status: 404 });
    ...
    const tempPassword = 'TempPass#' + Math.floor(Math.random() * 90000 + 10000).toString();
...
return NextResponse.json({ success: true, message: 'Password reset. User has been emailed their temporary password.' });
  } catch (error) {
